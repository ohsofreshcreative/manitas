...
<section data-gsap-anim="section" @if(!empty($section_id)) id="{{ $section_id }}" @endif class="b-steps -smt {{ $sectionClass }} {{ $section_class }}">
	<div class="__wrapper c-main">
		<div class="__content">
			<div data-gsap-element="header" class="__wrapper block w-full md:w-1/2 pb-10">
				<h3 class="">{{ $g_steps['title']}}</h3>
				<div class="">{!! $g_steps['text'] !!}</div>
			</div>

			<div class="swiper steps-swiper !overflow-visible">
				<div data-gsap-element="swiper" class="swiper-wrapper items-stretch">
					@foreach($r_steps as $card)
					<div class="swiper-slide flex !h-auto">
						<div class="__card relative bg-white radius h-full p-8">

							<div class="relative z-10 flex flex-col gap-4">
								@if (!empty($card['image']))
								<img class="max-w-[56px]" src="{{ $card['image']['url'] }}" alt="{{ $card['image']['alt'] ?? '' }}">
								@endif
								<h6 class="text-h7">{{ $card['header'] }}</h6>
